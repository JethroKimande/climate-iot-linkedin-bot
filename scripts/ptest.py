...
router_ip = "http://192.168.10.1"  # Common Tenda router IP
# update if different
username = "admin"  # Default username
password = "nimda12345"  # Your password

# Router Reboot URL (Tenda routers use a specific endpoint for rebooting,
# adjust as necessary)
router_reboot_url = f"{router_ip}/goform/reboot"  # Common Tenda reboot URL,
# but verify in your manual
...
def reboot_router():
    try:
        ...
        response = session.get(router_reboot_url)  # GET request to reboot
        if response.status_code == 200:
            print("Router rebooted successfully.")
        else:
            print(
                "Failed to reboot the router. "
...
def set_dns():
    ...
    os.system(
        f'netsh interface ip set dns name="{interface}" static {dns_servers}'
    )
